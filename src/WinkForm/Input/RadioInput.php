...
class RadioInput extends Input
{
    ...
    public function render()
    {
        ...
        // radio input has its options in the values array
        if (! empty($this->values))
        {
            if (! empty($this->renderInColumns))
                $rowsPerColumn = ceil(count($this->values) / $this->renderInColumns);
            else
                $rowsPerColumn = 1; // this is not being used, but at least this variable always exists.

            // determine values to display as selected
